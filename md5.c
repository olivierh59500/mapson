...
static u_int32_t SWAP(u_int32_t n)
    {
    static int have_endian = 0;
...
md5_read_ctx (ctx, resbuf)
     ...
     void *resbuf;
{
  ((u_int32_t *) resbuf)[0] = SWAP (ctx->A);
  ((u_int32_t *) resbuf)[1] = SWAP (ctx->B);
  ((u_int32_t *) resbuf)[2] = SWAP (ctx->C);
  ((u_int32_t *) resbuf)[3] = SWAP (ctx->D);
     ...
     void *resbuf;
{
  ...
  *(u_int32_t *) &ctx->buffer[bytes + pad] = SWAP (ctx->total[0] << 3);
  *(u_int32_t *) &ctx->buffer[bytes + pad + 4] = SWAP ((ctx->total[1] << 3) |
							(ctx->total[0] >> 29));
